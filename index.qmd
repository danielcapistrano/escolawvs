...
```{r}
library(gt)
resposta |> pluck("dados") |> as_tibble() |> gt()
```
...
Aqui está o conteúdo do [arquivo](https://www.camara.leg.br/robots.txt) localizado no site da camara dos deputados:
...
```{r}
library(rvest)
pagina <- read_html("https://www.camara.leg.br/noticias")
...
map_dfr(links[1:5],~{
    ...
    mutate(texto = substr(texto, 1, 600))
}) |> gt()
...
- Aumentar o número de caracteres do texto da noticia de 600 para 1000.  
- Adicionar uma coluna contendo o número de comentários de cada notícia  
- Identificar uma página que seja relevante para o grupo e pensar em formas de automatizar a coleta de dados.  
